...
* **renderers** - Object, Use { canvas: Canvas } with node canvas to do SSR
* **defaultColorSpace** - String, Use one of `hex` `rgb` or `hsl` to set `state.view` of the ChromeFields from a HOC.
* **onColorSpaceChange** - Function, callback for a HOC with  param of updated `state.view` for when ChromeFields toggle buttons are pressed.
