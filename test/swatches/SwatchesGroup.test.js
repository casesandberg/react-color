...
describe('SwatchesGroup', () => {
  ...
  it('should pass up data onClick', () => {
    ...
    const SwatchesGroup = TestUtils.renderIntoDocument(
      <SwatchesGroupComponent { ...props } group={ ['#333'] } />
    )
  ...
  it('should render SwatchesGroup for each top-level array passed to props.colors', () => {
    const SwatchesGroup = TestUtils.renderIntoDocument(
      <SwatchesGroupComponent { ...props } group={ ['#333', '#fff', '#aaa', '#ddd'] } />
    )
    const colors = TestUtils.scryRenderedComponentsWithType(SwatchesGroup, SwatchesColor)
