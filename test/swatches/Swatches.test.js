...
describe('Swatches', () => {
  ...
  it('should render SwatchesGroup for each top-level array passed to props.colors', () => {
    const SwatchesComponent = TestUtils.renderIntoDocument(
      <Swatches { ...props } colors={ [['#333'], ['#fff'], ['#aaa'], ['#ddd']] } />
    )
    const groups = TestUtils.scryRenderedComponentsWithType(SwatchesComponent, SwatchesGroup)
